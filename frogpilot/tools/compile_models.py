...
def download_models():
  ...
  for name in list_remote_onnx_files(session):
    ...
    download_file(CANCEL_DOWNLOAD_PARAM, destination, MODEL_DOWNLOAD_PARAM, params_memory, DOWNLOAD_PROGRESS_PARAM, session, url)

    if not verify_download(destination, params_memory, session, url):
      continue
