...
def get_device_type():
  ...
  device = model.split('comma ')[-1]
  if device not in ("tici", "tizi", "mici", "tici lite"):
    cloudlog.warning(f"Unknown device variant '{device}', treating as tici")
    return "tici"
  return device
...
class Tici(HardwareBase):
  ...
  def has_driver_camera(self) -> bool:
    return self.get_device_type() != "tici lite"
  ...
  def set_ir_power(self, percent: int):
    if self.get_device_type() in ("tici", "tizi"):
      return
    if not self.has_driver_camera():   # tici lite: no dcam, no IR LED
      return
