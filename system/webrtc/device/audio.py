...
class AudioInputStreamTrack(aiortc.mediastreams.AudioStreamTrack):
  @staticmethod
  def _audio_format_map(pyaudio_module):
    return {
      pyaudio_module.paUInt8: 'u8',
      pyaudio_module.paInt16: 's16',
      pyaudio_module.paInt24: 's24',
      pyaudio_module.paInt32: 's32',
      pyaudio_module.paFloat32: 'flt',
    }

  def __init__(self, audio_format: int | None = None, rate: int = 16000, channels: int = 1, packet_time: float = 0.020, device_index: int | None = None):
    super().__init__()
  ...
  async def recv(self):
    ...
    frame = av.AudioFrame.from_ndarray(mic_array, format=self._audio_format_map(self.pyaudio)[self.format], layout=layout)
