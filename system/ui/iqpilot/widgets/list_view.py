...
class IQMultipleButtonAction(MultipleButtonAction):
  ...
  def _render(self, rect: rl.Rectangle):
    ...
    highlight_color = style.MBC_BG_CHECKED_ENABLED if selected_enabled else style.MBC_DISABLED
    ...
    for i, _text in enumerate(self.buttons):
      ...
      text_color = style.ITEM_TEXT_COLOR if button_enabled else style.MBC_DISABLED
  ...
  def _handle_mouse_release(self, mouse_pos: MousePos):
    ...
    for i in range(len(self.buttons)):
      button_rect = rl.Rectangle(self._rect.x + i * self.button_width, button_y, self.button_width, style.BUTTON_HEIGHT)
