...
class SpeedLimitRenderer(Widget):
  def __init__(self):
    ...
    arrow_size = 90
    self._arrow_up = gui_app.texture("../../iqpilot/selfdrive/assets/img_plus_arrow_up.png", arrow_size, arrow_size)
    self._arrow_down = gui_app.texture("../../iqpilot/selfdrive/assets/img_minus_arrow_down.png", arrow_size, arrow_size)
  ...
  def update(self):
    ...
    if sm.updated["iqPlan"]:
      ...
      assist = lp_iq.speedLimit.assist
  ...
  def _render(self, rect: rl.Rectangle):
    ...
    info_rect = rl.Rectangle(x, y, width + 12, 160)
    ...
    if self.speed_limit_assist_state == AssistState.preActive:
      ...
      self._draw_sign_main(info_rect, alpha)
      self._draw_pre_active_arrow(info_rect)
    else:
      self.assist_frame = 0
      self._sign_alpha_filter.update(1.0)
      self._draw_sign_main(info_rect)
      self._draw_ahead_info(info_rect)
  ...
  def _draw_pre_active_arrow(self, sign_rect):
    ...
    arrow_x = sign_rect.x + (sign_rect.width - self._arrow_up.width) / 2
