...
def check_panda_support(panda) -> bool:
  ...
  if hw_type == Panda.HW_TYPE_UNO:
    cloudlog.warning("mr.one C3 clone panda detected (HW_TYPE_UNO), skipping flash")
    return False
...
def main() -> None:
  ...
  while not do_exit:
    try:
      ...
      for panda in pandas:
        # skip health check only for truly unsupported hw types
