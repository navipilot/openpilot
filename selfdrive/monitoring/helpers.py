...
class DriverMonitoring:
  def __init__(self, rhd_saved=False, settings=None, always_on=False, no_driver_camera=False):
    ...
    self.no_driver_camera = no_driver_camera
  ...
  def run_step(self, sm, demo=False):
    if self.no_driver_camera:
      # tici lite: no dcam — wheel-touch monitoring only, face detection skipped entirely
      ...
      self.face_detected = False
      self.driver_distracted = False
      self._set_policy(brake_disengage_prob=0.0, car_speed=highway_speed)
      self._update_events(driver_engaged=driver_engaged, op_engaged=enabled,
                          standstill=standstill, wrong_gear=wrong_gear, car_speed=highway_speed)
      return
