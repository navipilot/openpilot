...
def update_steer_fault(events, *, standstill=True, temporary=True, permanent=False):
...
def test_moving_tesla_temporary_fault_is_not_debounced():
  ...
  assert EventName.steerTempUnavailableSilent in update_steer_fault(events, standstill=False).names
