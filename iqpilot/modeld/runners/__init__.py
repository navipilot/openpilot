...
from openpilot.iqpilot.modeld.runners.runmodel_pyx import RunModel, Runtime
assert Runtime
...
class ModelRunner(RunModel):
  ...
  def __new__(cls, paths, *args, **kwargs):
    if ModelRunner.THNEED in paths and USE_THNEED:
      from openpilot.iqpilot.modeld.runners.thneedmodel_pyx import ThneedModel as Runner
      runner_type = ModelRunner.THNEED
    elif ModelRunner.ONNX in paths:
      from openpilot.iqpilot.modeld.runners.onnxmodel import ONNXModel as Runner
