...
class TestNNFFFingerprintBase:
  ...
  def test_exact_fingerprint(self, car_name):
    ...
    assert CI.CP_IQ.neuralNetworkFeedForward.model.name != "MOCK" and not CI.CP_IQ.neuralNetworkFeedForward.fuzzyFingerprint

  @parameterized.expand(FINGERPRINT_FUZZY_MATCH)
  def test_fuzzy_fingerprint(self, car_name):
    CI = self._setup_platform(car_name)
    assert CI.CP_IQ.neuralNetworkFeedForward.model.name != "MOCK" and CI.CP_IQ.neuralNetworkFeedForward.fuzzyFingerprint

  @parameterized.expand(FINGERPRINT_ANGLE_NO_MATCH)
  def test_no_fingerprint(self, car_name):
    CI = self._setup_platform(car_name)
    assert CI.CP_IQ.neuralNetworkFeedForward.model.name == "MOCK"
