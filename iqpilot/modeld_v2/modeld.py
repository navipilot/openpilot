  ...
  while True:
    ...
    if sm.updated["liveCalibration"] and sm.seen['roadCameraState'] and sm.seen['deviceState']:
      ...
      if device_from_calib_euler is None:
        # Use rpyCalib directly even when not yet calibrated — starts at [0,0,0]
        # and converges gradually. This avoids the systematic pitch/yaw bias that
        # an identity warp (zeros) introduces when the device is mounted at a
        # non-zero angle, while still providing a valid transform so that
        # cameraOdometry.valid=True and locationd can initialize.
    ...
    if model_output is not None:
      ...
      mdv2iq_send.iqModelDataV2.laneTurnDirection = DH.nav_turn_direction if DH.nav_turn_direction != TurnDirection.none else DH.lane_turn_direction
